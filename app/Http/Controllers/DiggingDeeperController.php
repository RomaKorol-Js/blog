...
class DiggingDeeperController extends Controller
{
    ...
    public function collections()
    {
        ...
        // dd(__METHOD__, $eloquentCollection, $eloquentCollection->toArray()); // колекція має більше елементів функцій та іншого в той час як масим займає менше місця 
    ...
    //         $collection
    //     );// Робить те саме що масив але із колекціями
        ...
        // dd ($result); // повертаємо із фільтрами
        ...
        // dd ($collection);  // повертаємо лише потірбні рядки 
        ...
        // dd ($newItem, $newItem2); //повертаємо прості об'єкти
        ...
        // dd(compact('collection', 'newItemFirst' , 'newItemLast', 'pulledItem')); // виводимо ці значення із новими елементами 

        //Фільтрація
        $filtered = $collection->filter(function ($item) {
            if (!isset($item->created_at)) return false;
        ...
        // dd(compact('filtered')); //закоментувати 91-106 рядки перед перевіркою //жодний обєкт не створений в пятницю 11 тож мені варто змінити умову щоб отримати не пусту колекцію

        $sortedSimpleCollection = collect([5, 3, 1, 2, 4])->sort()->values();
        $sortedAscCollection = $collection->sortBy('created_at');
        $sortedDescCollection = $collection->sortByDesc('item_id');

        // dd(compact('sortedSimpleCollection', 'sortedAscCollection', 'sortedDescCollection')); // сортовано за песними ознаками
